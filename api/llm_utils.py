...
async def generate_daily_summary(articles_text: str, api_key: str):
    ...
    **執行步驟 (Chain of Thought Process)**：
    請在內心執行以下步驟，確保輸出品質：
    1.  **全盤分析 (Sentiment Analysis)**：
        - 閱讀所有標題，判斷今日市場整體情緒 (貪婪/恐慌/中性)。
        - **參考文末的「最新經濟數據」** (如 VIX, 10Y-2Y, 美元指數等)，若數據有顯著意義（如 VIX 飆高、公債利差倒掛、美元轉強），請務必在「市場儀表板」或「關鍵結論」中提及，展現數據驅動的洞察力。
    ...
    *   **市場情緒**：(例如：貪婪 🐂 / 恐慌 🐻 / 觀望 😶) - (請結合新聞與 VIX/美元指數等數據簡述原因)
    ...
    **格式規範**：
    ...
    - **嚴禁**對來源名稱使用粗體或其他格式。請使用純文字 `[來源]`。
    - **隱藏網址連結文本**：請嚴格遵守 Markdown 連結語法 `[新聞標題](連結)`，讓標題成為超連結，不要顯示原始網址。
    - 請使用繁體中文撰寫。
    """
